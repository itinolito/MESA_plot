...
for mass_folder in mass_folders:
   ...
   ax1.annotate(r'$\times 10^{}$'.format(exponent_axis),
          xy=(1.02, .04), xycoords='axes fraction', fontsize=12)
     # else:
      #   continue

   #ax1.scatter(relevant_ages,relevant_radius, c="red", zorder=1)
   #for k,j in enumerate(he_folders):
   #   ax1.annotate('{}'.format(j),xy=(relevant_ages[k],relevant_radius[k]),fontsize=8,xytext=(-10-k*25,-27),textcoords='offset points',arrowprops=dict(arrowstyle="->",connectionstyle="arc3",lw=0.5))
...
leg = fig.legend(handles, labels, fontsize="x-small", ncol = 5, loc="lower center")
